    ...
    for (int i = 0; i < ROW - 1; i++) {
        for (int j = 0; j < COLUMN - 1; j++) {
            for (int k = j + 1; k < COLUMN; k++) {
                if ((arr[i][j] == arr[i][k]) && (arr[i][j] != a[0])) check++;
            }
        }
    }
    if (check == 0) return 0;
...
int check_side(char arr[ROW][COLUMN]) {
    ...
    for (int i = 1; i < ROW - 2; i++) {
        for (int j = 0; j < COLUMN; j++) {
            if (((arr[i][j] == arr[i - 1][j]) || (arr[i][j] == arr[i + 1][j])) && (arr[i][j] != a[0])) check++;
        }
    }
    if (check == 0) return 0;
    else return 1;
...
int check_corner(char arr[ROW][COLUMN]) {
    ...
    for (int i = 0; i < ROW - 2; i++) {
        for (int j = 1; j < COLUMN - 1; j++) {
            if ( ((arr[i][j] == arr[i + 1][j - 1]) || (arr[i][j] == arr[i + 1][j + 1])) && (arr[i][j] != a[0])) check++;
        }
    }
